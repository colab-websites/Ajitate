...
export function RestaurantMenu({onSelect,onQuickAdd}:{onSelect:(id:string)=>void;onQuickAdd:(id:string)=>void}) {
  ...
  return <>
    <section id="menu" className="restaurant-menu">
      ...
      <p className="menu-result" role="status">{visible.length} opciones{search?` para “${search}”`:''}</p><div id="menu-products" className={`food-grid mobile-preview ${expanded||search?'is-expanded':''}`}>{visible.map(card)}</div>{visible.length>2&&!search&&<button className="glass-action mobile-show-more" aria-expanded={expanded} aria-controls="menu-products" onClick={()=>setExpanded(v=>!v)}>{expanded?'Ver menos ↑':`Ver las ${visible.length} opciones ↓`}</button>}{!visible.length&&<p className="empty-cart">No encontramos ese antojo. Prueba con otro nombre.</p>}
      <p className="menu-source">Precios del menú compartido. Disponibilidad y costo de entrega se confirman con el restaurante.</p>
    </section>
    <section id="especiales" className="restaurant-menu specials-menu"><p className="section-kicker">PARA DARTE UN GUSTO</p><h2>NUESTROS <span>ESPECIALES</span></h2><figure className="menu-category-photo"><img src={`${import.meta.env.BASE_URL}images/specials.png`} alt="Imagen ilustrativa de costillas BBQ con papas" loading="lazy"/><figcaption>Imagen de categoría creada con IA · ilustrativa, no fotografía del plato real</figcaption><div><span>BBQ, PARA COMPARTIR</span><strong>Mucho sabor.</strong></div></figure><h3 className="special-group">Edición especial</h3><div className="food-grid">{MENU.filter(i=>i.category==='Edición especial').map(card)}</div><h3 className="special-group">Especiales & acompañamientos</h3><div id="special-products" className={`food-grid mobile-preview ${specialsExpanded?'is-expanded':''}`}>{MENU.filter(i=>i.category==='Especiales').map(card)}</div><button className="glass-action mobile-show-more" aria-expanded={specialsExpanded} aria-controls="special-products" onClick={()=>setSpecialsExpanded(v=>!v)}>{specialsExpanded?'Ver menos ↑':'Ver todos los acompañamientos ↓'}</button></section>
  </>
}
